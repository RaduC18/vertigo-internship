...
function MarketDetailPage() {
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      setIsBetting(true);
      setError(null);
      const result = await api.placeBet(marketId, selectedOutcomeId, parseFloat(betAmount));
      setBetAmount("");
          ...
          <CardContent className="space-y-6">
                    ...
                    <PieChart>
                      ...
                      >
                        {market.outcomes.map((_, index) => (
                          <Cell
                            key={index}
                            fill={["#6366f1", "#22c55e", "#f59e0b", "#ef4444"][index % 4]}
                          />
                        ))}
                      </Pie>
            ...
            {user?.isAdmin && market.status === "active" && (
              <Card className="border-red-200 bg-red-50">
                <CardHeader>
                  <CardTitle className="text-red-700">🔧 Admin Controls</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <p className="text-sm font-medium text-red-700">Resolve Market — select winning outcome:</p>
                    <div className="space-y-2">
                      {market.outcomes.map((outcome) => (
                        <Button
                          key={outcome.id}
                          ...
                          onClick={async () => {
                            if (confirm(`Resolve market with "${outcome.title}" as winner?`)) {
                              try {
                                await api.resolveMarket(market.id, outcome.id);
                                const updated = await api.getMarket(market.id);
                                setMarket(updated);
                              } catch (err) {
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      if (confirm("Archive this market and return all funds to bettors?")) {
                        try {
